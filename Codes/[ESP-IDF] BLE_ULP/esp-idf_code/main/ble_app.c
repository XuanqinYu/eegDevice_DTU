...
#define TAG "BLE_CFG"
extern uint8_t ads1115_mode;


//设备名称
#define BLE_DEVICE_NAME     "EEG_BLE"
...
#define MAX_PACKET_SIZE  514  // 每次发送最多500字节
...
static const uint16_t GATTS_SERVICE_UUID_TEST       = 0x18FF;   //Service: EEG Data
static const uint16_t GATTS_CHAR_UUID_CH1           = 0x2AFE;   //Chara1: value
static const uint16_t GATTS_CHAR_UUID_CH2           = 0x2A57;   //Chara2: mode setting (digital output)
...
uint16_t gl_conn_id = 0xFFFF;

//char1的值
static char sv1_char1_value[20] = {0};
//char2的值
static char sv1_char2_value[1] = {0x00};
...
static esp_ble_adv_params_t adv_params = {
    ...
    // .adv_int_min         = 0x400,                //最小广播间隔，单位:0.625ms
    // .adv_int_max         = 0x800,                //最大广播间隔，单位:0.625ms
...
static const esp_gatts_attr_db_t gatt1_db[SV1_IDX_NB] =
{
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_declaration_uuid, ESP_GATT_PERM_READ,
      sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&char_prop_read_notify}},   //特征值1允许读通知
    //特征值
    [SV1_CH1_IDX_CHAR_VAL] =
    {{ESP_GATT_RSP_BY_APP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_UUID_CH1, ESP_GATT_PERM_READ, 
    ...
    [SV1_CH2_IDX_CHAR]      =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_declaration_uuid, ESP_GATT_PERM_READ,
      sizeof(uint8_t), sizeof(uint8_t), (uint8_t *)&char_prop_read_write_notify}},  //特征值2允许读写和通知
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
        ...
        case ESP_GATTS_READ_EVT:    //收到客户端的读取事件
        {
            ...
            if(sv1_handle_table[SV1_CH1_IDX_CHAR_VAL] == param->read.handle) // EEG data
            {
                rsp.attr_value.len = sizeof(sv1_char1_value);
                memcpy(rsp.attr_value.value,sv1_char1_value,sizeof(sv1_char1_value));
            }
            if(sv1_handle_table[SV1_CH2_IDX_CHAR_VAL] == param->read.handle) // settig mode
            {
        ...
        case ESP_GATTS_WRITE_EVT:   //收到客户端的写事件
            if (!param->write.is_prep){
                ...
                else if(param->write.handle == sv1_handle_table[SV1_CH2_IDX_CHAR_VAL])   //特征2的值
                {
                    ...
                    ads1115_mode = sv1_char2_value[0];
                    // ESP_LOGI(TAG, "channel number: %d", sv1_char2_value[0]);    //收到命令，输出收到的值
                }
        ...
        case ESP_GATTS_CONNECT_EVT:     //收到连接事件
            ...
            // conn_params.latency = 30;       //从机延迟
            // conn_params.max_int = 0xC80;    // 最大连接间隔 = 0x20*1.25ms = 40ms
            // conn_params.min_int = 0x640;    // 最小连接间隔 = 0x10*1.25ms = 20ms
            // conn_params.timeout = 6000;    // 监控超时 = 400*10ms = 4000ms
            ...
            // 停止BLE广播，防止高功耗
            // esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_ADV, ESP_PWR_LVL_N12); // 降低广播功率
            // esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_DEFAULT, ESP_PWR_LVL_N12); // 降低默认功率
            esp_err_t ret = esp_ble_gap_stop_advertising();
            ESP_LOGI(TAG, "Stop advertising result: %s", (ret == ESP_OK) ? "Success" : "Failed");
            // ads1115_mode = 1;
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    switch (event) {
        ...
        //     // BLE连接后禁止深度睡眠
        //     spinlock_acquire(light_sleep_lock);
        //     break;
        // case ESP_GATTS_DISCONNECT_EVT:
        //     // 断开后恢复低功耗
        //     spinlock_release(light_sleep_lock);
        //     break;
...
esp_err_t ble_cfg_net_init(void)
{
    ...
    bt_cfg.ble_max_conn  = 1;       // 仅保留1个活动连接
    ...
    // ESP_ERROR_CHECK(esp_ble_gattc_send_mtu_req(gatts_if, 400));
    esp_ble_scan_params_t scan_params = {
        .scan_type = BLE_SCAN_TYPE_PASSIVE,
        .own_addr_type = BLE_ADDR_TYPE_PUBLIC,
        .scan_filter_policy = BLE_SCAN_FILTER_ALLOW_ALL,
        .scan_interval = 0x100,     // 扫描间隔
        .scan_window = 0x50         // 扫描窗口
    };
    ESP_ERROR_CHECK(esp_ble_gap_set_scan_params(&scan_params));
    esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_DEFAULT, ESP_PWR_LVL_N12);  // 全局默认功率
    esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_ADV, ESP_PWR_LVL_N12);       // 广播功率
    esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_SCAN, ESP_PWR_LVL_N9);       // 连接功率
...
void ble_send_data(uint8_t *data, uint16_t length)
{
    if(gl_conn_id != 0xFFFF && (sv1_ch1_client_cfg[0] | sv1_ch1_client_cfg[1]))
    {
        esp_ble_gatts_set_attr_value(sv1_handle_table[SV1_CH1_IDX_CHAR_VAL], length, data);
        esp_ble_gatts_send_indicate(gl_gatts_if, gl_conn_id, sv1_handle_table[SV1_CH1_IDX_CHAR_VAL], length, data, false);
    }
}

// 每次 MAX_PACKET_SIZE 字节
void ble_set_ch1_value_large(uint8_t *data, uint16_t total_length)
{
    uint16_t offset = 0;
    while (offset < total_length)
    {
        uint16_t chunk_size = (total_length - offset > MAX_PACKET_SIZE) ? MAX_PACKET_SIZE : (total_length - offset);
        ble_send_data(data + offset, chunk_size);
        offset += chunk_size;
        vTaskDelay(pdMS_TO_TICKS(20)); // 避免数据过快导致丢失
    }
}
